...
async def get_user(db: AsyncSession,username: str) -> List[user_schema.User]:
    result: Result = await(
        db.execute(
            select(
                user_model.User.id,
...
async def get_current_active_user(token: str = Depends(oauth2_scheme), current_user:User = Depends(get_current_user)):
