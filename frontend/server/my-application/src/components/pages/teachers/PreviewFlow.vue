...
export default {
  ...
  methods:{
        ...
        axios.post(`http://localhost:8000/start_new_flow_session`, params, config)
          .then(function(response){
            ...
            if(response.data.start_success){
              self.$router.push({name:'PreviewFlowSession', params: {flow_session_id: response.data.flow_session_id, page_num: 1}})
            }
          }).catch(
            function(error){
              console.log(error)
      ...
      restart_flow_session(flow_session_id){
        this.$router.push({name:'PreviewFlowSession', params: {flow_session_id: flow_session_id, page_num: 1}})
      },
  },
};
</script>
